...
const Group = () => {
    return (
        <div className='border border-border  flex flex-col gap-2 rounded-2xl p-2 sm:p-4'>
            <div className='font-bold md:text-xl sm:text-sm text-xs'>Groups</div>

            {Groups.map((item, index) => (
                <div key={index} className={`flex gap-2 py-1 sm:py-2.5 ${index !== Groups.length - 1 ? 'sm:border-b border-border' : ''}`}>
